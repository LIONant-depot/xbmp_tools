...
namespace xbmp::tools::lighting
{
    ...
    xmath::fvec2 integrateDFG(float NoV, float linearRoughness, size_t numSamples)
    {
        ...
        std::vector<xmath::fvec2> samples(numSamples);
        for (size_t i = 0; i < numSamples; ++i) {
            samples[i] = hammersley(static_cast<uint32_t>(i), invN);
        }
#pragma omp parallel for reduction(+:sumX,sumY)
        for (int i = 0; i < static_cast<int>(numSamples); ++i)
        {
            const xmath::fvec2 u = samples[i];
...
#pragma omp parallel for
    ...
    int get_bytes_per_pixel(xbitmap::format fmt)
    {
        switch (fmt)
        {
        case xbitmap::format::R8G8B8A8: return 4;
        case xbitmap::format::R8G8B8U8: return 3;
        case xbitmap::format::R16G16B16A16_SFLOAT: return 8;
        case xbitmap::format::R32G32B32_FLOAT: return 12;
        case xbitmap::format::R32G32B32A32_FLOAT: return 16;
        default: assert(false); return 0;
        }
    }
    int get_channels(xbitmap::format fmt)
    {
        switch (fmt)
        {
        case xbitmap::format::R8G8B8A8:
        case xbitmap::format::R16G16B16A16_SFLOAT:
        case xbitmap::format::R32G32B32A32_FLOAT: return 4;
        case xbitmap::format::R8G8B8U8:
        case xbitmap::format::R32G32B32_FLOAT: return 3;
        default: assert(false); return 0;
        }
    }
    xmath::fvec3 sampleCubemapBilinear(const xbitmap& cubemap, const xmath::fvec3& dir, float mipLevel = 0.0f)
    {
        mipLevel = std::max(0.0f, mipLevel);
        CubemapFaceInfo info = getCubemapFace(dir.NormalizeSafeCopy());
        int mip0 = static_cast<int>(std::floor(mipLevel));
        float fracMip = mipLevel - static_cast<float>(mip0);
        int mip1 = mip0 + 1;
        const int maxMip = cubemap.getMipCount() - 1;
        mip0 = std::min(mip0, maxMip);
        mip1 = std::min(mip1, maxMip);
        auto sampleMip = [&](int mip) -> xmath::fvec3 {
            ...
            int x1 = x0 + 1;
            int y1 = y0 + 1;
            auto getColor = [&](int x, int y, int m) -> xmath::fvec3
                {
                    ...
                    if (is_int)
                    {
                        ...
                        auto span = cubemap.getMip<xcolori>(m, info.face);
                    ...
                    else if (is_half)
                    {
                        ...
                        auto span = cubemap.getMip<uint16_t>(m, info.face);
                    ...
                    else // float32
                    {
                        ...
                        auto span = cubemap.getMip<float>(m, info.face);
            ...
            xmath::fvec3 c00 = getColor(x0, y0, mip);
            xmath::fvec3 c10 = getColor(x1, y0, mip);
            xmath::fvec3 c01 = getColor(x0, y1, mip);
            xmath::fvec3 c11 = getColor(x1, y1, mip);
            xmath::fvec3 c0 = c00 * (1 - frac[0]) + c10 * frac[0];
            xmath::fvec3 c1 = c01 * (1 - frac[0]) + c11 * frac[0];
            return c0 * (1 - frac[1]) + c1 * frac[1];
            };
        xmath::fvec3 color0 = sampleMip(mip0);
        xmath::fvec3 color1 = sampleMip(mip1);
        return color0 * (1.0f - fracMip) + color1 * fracMip;
    }
    ...
    void write_pixel(std::byte* ptr, const xmath::fvec3& color, float alpha, bool is_int, bool is_half, int channels)
    {
        xmath::fvec3 out_color = color;
        float out_alpha = alpha;
        if (is_int)
        {
            out_color = xmath::fvec3::Max(out_color, xmath::fvec3(0.0f));
            out_color = xmath::fvec3(std::pow(out_color.m_X, 1.0f / 2.2f), std::pow(out_color.m_Y, 1.0f / 2.2f), std::pow(out_color.m_Z, 1.0f / 2.2f));
            out_color *= 255.0f;
            out_color = xmath::fvec3::Min(out_color, xmath::fvec3(255.0f));
            out_alpha = std::min(std::max(out_alpha * 255.0f, 0.0f), 255.0f);
        }
        if (is_int)
        {
            uint8_t* uptr = reinterpret_cast<uint8_t*>(ptr);
            uptr[0] = static_cast<uint8_t>(out_color.m_X);
            uptr[1] = static_cast<uint8_t>(out_color.m_Y);
            uptr[2] = static_cast<uint8_t>(out_color.m_Z);
            if (channels == 4) uptr[3] = static_cast<uint8_t>(out_alpha);
        }
        else if (is_half)
        {
            uint16_t* hptr = reinterpret_cast<uint16_t*>(ptr);
            hptr[0] = xmath::half(out_color.m_X).m_Value;
            hptr[1] = xmath::half(out_color.m_Y).m_Value;
            hptr[2] = xmath::half(out_color.m_Z).m_Value;
            if (channels == 4) hptr[3] = xmath::half(out_alpha).m_Value;
        }
        else
        {
            float* fptr = reinterpret_cast<float*>(ptr);
            fptr[0] = out_color.m_X;
            fptr[1] = out_color.m_Y;
            fptr[2] = out_color.m_Z;
            if (channels == 4) fptr[3] = out_alpha;
        }
    }
    ...
    )
    {
        ...
        xbitmap::format outputFmt = inputCubemap.getFormat();
        int bytes_per_pixel = get_bytes_per_pixel(outputFmt);
        int channels = get_channels(outputFmt);
        ...
        const bool output_is_int = input_int;
        const bool output_is_half = input_half;
        const bool output_is_float32 = input_float32;
        const int input_channels = channels; // same
        const int numMips = nMips < 0 ? xbmp::tools::mips::ComputeMaxMips(1, baseSize, baseSize) : nMips;
        const int maxInputMips = xbmp::tools::mips::ComputeMaxMips(1, baseSize, baseSize);
        assert(inputCubemap.getMipCount() >= maxInputMips);
        const xbitmap* sampleMap = &inputCubemap;
        // Precompute hammersley for specular
        std::vector<xmath::fvec2> specularXi(specularSamples);
        const float invSpec = 1.0f / static_cast<float>(specularSamples);
        for (int s = 0; s < specularSamples; ++s) {
            specularXi[s] = hammersley(static_cast<uint32_t>(s), invSpec);
        }
        // Compute total work
        float total_work = 0.0f;
        float specular_work = 0.0f;
        for (int level = 1; level < numMips; ++level) {
            int size = baseSize >> level;
            specular_work += 6.0f * size * size * specularSamples;
        }
        total_work += specular_work;
        float diffuse_work = 0.0f;
        if (diffuse_type == diffuse_type::CUBEMAP && diffuseCubemap) {
            diffuse_work = 6.0f * diffuseRes * diffuseRes * diffuseSamples;
            total_work += diffuse_work;
        }
        else if (diffuse_type == diffuse_type::SH && shCoeffs) {
            diffuse_work = 6.0f * baseSize * baseSize * 1.0f; // approximate, no heavy sampling
            total_work += diffuse_work;
        }
        if (total_work == 0.0f) total_work = 1.0f;
        float done_work = 0.0f;
        ...
        // Copy mip 0 from input
        for (int face = 0; face < 6; ++face)
        {
            auto dst_span = specularOutput.getMip<std::byte>(0, face);
            auto src_span = inputCubemap.getMip<std::byte>(0, face);
            std::memcpy(dst_span.data(), src_span.data(), static_cast<size_t>(baseSize) * baseSize * bytes_per_pixel);
        }
        // Generate specular mips
        const float omega_t = 4.0f * F_PI / (6.0f * static_cast<float>(baseSize) * static_cast<float>(baseSize));
        for (int level = 1; level < numMips; ++level)
        {
                ...
                for (int y = 0; y < cur_size; ++y)
                {
                    for (int x = 0; x < cur_size; ++x)
                    {
                        ...
                        for (int s = 0; s < specularSamples; ++s)
                        {
                            ...
                            if (NoL > 0.0f)
                            {
                                float NoH = saturate(N.Dot(H));
                                float pdf = D_GGX(NoH, roughness) * NoH / (4.0f * NoH + 0.001f);
                                float omega_s = 1.0f / (static_cast<float>(specularSamples) * pdf + 0.0001f);
                                float mipLevel = roughness == 0.0f ? 0.0f : std::max(0.0f, 0.5f * std::log2(omega_s / omega_t));
                                xmath::fvec3 sampleColor = sampleCubemapBilinear(*sampleMap, L, mipLevel);
                        ...
                        std::byte* ptr = &dst_span[(y * cur_size + x) * bytes_per_pixel];
                        write_pixel(ptr, color, 1.0f, output_is_int, output_is_half, channels);
                    }
                }
            }
            // Update progress after each level
            float level_work = 6.0f * cur_size * cur_size * specularSamples;
            done_work += level_work;
            if (progressCallback) progressCallback(done_work / total_work);
        }
        // Diffuse
        if (diffuse_type == diffuse_type::CUBEMAP && diffuseCubemap != nullptr)
        {
            // Precompute hammersley for diffuse
            std::vector<xmath::fvec2> diffuseXi(diffuseSamples);
            const float invDiff = 1.0f / static_cast<float>(diffuseSamples);
            for (int s = 0; s < diffuseSamples; ++s) {
                diffuseXi[s] = hammersley(static_cast<uint32_t>(s), invDiff);
            }
...
#pragma omp parallel for
                for (int y = 0; y < diffuseRes; ++y)
                {
                    for (int x = 0; x < diffuseRes; ++x)
                    {
                        ...
                        for (int s = 0; s < diffuseSamples; ++s)
                        {
                            xmath::fvec2 Xi = diffuseXi[s];
                            xmath::fvec3 L = importanceSampleCosine(Xi, N);
                            float NoL = std::max(0.0f, N.Dot(L));
                            float pdf = NoL / F_PI;
                            float omega_s = 1.0f / (static_cast<float>(diffuseSamples) * pdf + 0.0001f);
                            float mipLevel = std::max(0.0f, 0.5f * std::log2(omega_s / omega_t));
                            color += sampleCubemapBilinear(*sampleMap, L, mipLevel);
                        }
                        color = color * static_cast<float>(PI) / static_cast<float>(diffuseSamples);
                        std::byte* ptr = &dst_span[(y * diffuseRes + x) * bytes_per_pixel];
                        write_pixel(ptr, color, 1.0f, output_is_int, output_is_half, channels);
                    }
                }
            }
            // Update progress after diffuse cubemap (since parallel on faces, update once at end)
            done_work += diffuse_work;
            if (progressCallback) progressCallback(done_work / total_work);
        }
        else if (diffuse_type == diffuse_type::SH && shCoeffs != nullptr)
        {
            ...
            std::vector<xmath::fvec3> localSh(9, xmath::fvec3(0.0f));
#pragma omp parallel
            {
                std::vector<xmath::fvec3> threadSh(9, xmath::fvec3(0.0f));
                    ...
                    if (input_int)
                    {
                        ...
                        for (int y = 0; y < res; ++y)
                        {
                            for (int x = 0; x < res; ++x)
                            {
                                ...
                                float y00 = 0.282095f;
                                float y1n1 = 0.488603f * dir[1];
                                float y10 = 0.488603f * dir[2];
                                float y1p1 = 0.488603f * dir[0];
                                float y2n2 = 1.092548f * dir[0] * dir[1];
                                float y2n1 = 1.092548f * dir[1] * dir[2];
                                float y20 = 0.315392f * (3.0f * dir[2] * dir[2] - 1.0f);
                                float y2p1 = 1.092548f * dir[0] * dir[2];
                                float y2p2 = 0.546274f * (dir[0] * dir[0] - dir[1] * dir[1]);
                                std::array<float, 9> basis = { y00, y1n1, y10, y1p1, y2n2, y2n1, y20, y2p1, y2p2 };
                                for (int k = 0; k < 9; ++k)
                                {
                                    threadSh[k] += color * basis[k] * solidAngleAvg;
                                }
                            }
                        }
                    }
...
#pragma omp critical
                {
                    for (int k = 0; k < 9; ++k) {
                        (*shCoeffs)[k] += threadSh[k];
                    }
                }
            }
            // Update progress after SH (since parallel on faces, update once at end)
            done_work += diffuse_work;
            if (progressCallback) progressCallback(done_work / total_work);
        }
...
////////////////////////////// FIRST OPTIMIZE VERSION DID NOT HELP MUCH....
/*
#include <cmath>
#include <algorithm>
#include <cstdint>
...
//////////////////////////////////////// ALLL HDR CUBE FILTER ISSUES SOLVE... RGB8 ISSSUES REMAIN...
/*
#include <cmath>
#include <algorithm>
#include <cstdint>
                    ...
                    for (int x = 0; x < cur_size; ++x)
                    {
                        ...
                        for (int s = 0; s < specularSamples; ++s)
                        {
                            ...
                            if (NoL > 0.0f)
                            {
                                ...
                                float pdf = D_GGX(NoH, roughness) * NoH / (4.0f * NoH + 0.001f);
                                float omega_s = 1.0f / (static_cast<float>(specularSamples) * pdf + 0.0001f);
                                float mipLevel = roughness == 0.0f ? 0.0f : 0.5f * std::log2(omega_s / omega_t);
                ...
                for (int y = 0; y < diffuseRes; ++y)
                {
                    for (int x = 0; x < diffuseRes; ++x)
                    {
                        ...
                        for (int s = 0; s < diffuseSamples; ++s)
                        {
                            ...
                            float pdf = NoL / F_PI;
                            float omega_s = 1.0f / (static_cast<float>(diffuseSamples) * pdf + 0.0001f);
                            float mipLevel = 0.5f * std::log2(omega_s / omega_t);
...
//////////////////////////////////// SOLVED THE WORNG FACING CUBE FACES
/*
#include <cmath>
#include <algorithm>
#include <cstdint>
